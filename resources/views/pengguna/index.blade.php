    ...
    <main class="flex-grow pt-20">
        <section id="beranda" class="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-slate-900">
            <div class="absolute inset-0 z-0">
                ...
                <img src="{{ asset('img/DcAsli.jpg') }}" alt="Kantor Kelurahan Dunguscariang" class="w-full h-full object-cover object-center scale-105">
            </div>
                    ...
                    @forelse ($konfirmasi as $data)
                        <article class="bg-slate-50 rounded-2xl overflow-hidden border border-slate-100 hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                            <div class="relative overflow-hidden h-56 bg-slate-200">
                                ...
                                <img src="{{ asset('img/konfirmasi/' . $data->foto) }}" alt="Informasi Kelurahan Dunguscariang" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            </div>
                            <div class="p-6 flex-grow flex flex-col justify-between">
                                <div>
                                    <span class="text-xs font-semibold text-slate-500 flex items-center gap-1.5 mb-2">
                                        <i data-lucide="calendar" class="w-3.5 h-3.5"></i> {{ optional($data->created_at)->format('d M Y') ?? 'Tanggal belum tersedia' }}
                                    </span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    @foreach($layanan as $item)
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/50 flex flex-col justify-between">
                            ...
                            <button type="button" onclick="openModal('{{ $item['key'] }}')" class="mt-6 w-full py-2 bg-slate-50 hover:bg-brand-50 hover:text-brand-700 text-slate-700 font-bold text-xs uppercase tracking-wider rounded-lg transition-colors border border-slate-200">
                                Lihat Berkas Syarat
                            </button>
                        </div>
                    @endforeach
    ...
    <footer id="kontak" class="bg-slate-950 text-white border-t border-slate-900 pt-16 pb-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="border-t border-slate-900 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
                <p>&copy; {{ date('Y') }} Pemerintah Kelurahan Dunguscariang. Hak Cipta Dilindungi.</p>
